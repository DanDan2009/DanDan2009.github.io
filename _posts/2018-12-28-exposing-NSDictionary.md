...
无论参数定义为const id *还是const id[]，都没有关系，因为数组在作为函数参数传递时将衰减为指针。
...
如果符号是本地的(非外部的)，则符号的类型由相应的小写字母表示。
...
__nsdictionarysize的符号并不导出——它们是用于库的内部使用的。我做了一些研究，想知道是否可以链接到非导出的符号，但显然不可能(请告诉我是否可以!)我们无法访问它们。也就是说，我们无法轻易地访问它们。
...
如果你搞砸了，__nsdictionari可能返回nil key!
